...
## disclaimers
* right now if `/tmp/mpvSockets` does not exist then videos launched through umpv, will not have a socket full stop. if mpv is launched on it's own then this script will create that directory, but (at least in my testing) no socket will actually be created, only the directory, which is somewhat odd to me. so as long as you launch standard mpv at least once then everything will work fine after that. as far as i can tell this is the only major issue currently present due to my modifications.
